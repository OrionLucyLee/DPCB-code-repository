...
attr2=2.^(0:1:16);                            % loading concentration of serine
...
xlabel('x (\mum)'); xlim([0,740]); xticks([0,200,400,600]);
...
figure('color','w');  plot(x,Rho','linewidth',2); ylim([0 0.005]);  xlim([0 740]); xlabel('x (\mum)'); ylabel('Pdf');  
...
for i=1:size(Rho,1)-1
    ...
    Ex2(i)=sqrt(sum((x(ind)-max(x)/2).^2.*(Rho(i,ind)'-Me(i)))./max(x).^2); 
end

figure('color','w'); subplot(2,1,1); semilogx(attr2(3:end-2),Ex1(3:end-1),'ob'); 
line(2.^(2:0.2:14),[0.3*ones(1,61);0.7*ones(1,61)]); 
ylim([0,1]); xlim([3,32000]); ylabel('w_1');
hold on; subplot(2,1,2);semilogx(attr2(3:end-2),Ex2(3:end-1),'or'); 
line(2.^(2:0.2:14),0.15*ones(1,61)); ylim([0,0.5]);  ylabel('w_2'); 
xlim([3,32000]); xlabel('Serine conc. (\muM)');